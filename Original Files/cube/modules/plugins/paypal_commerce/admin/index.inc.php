...
if (!empty($client_id) && !empty($client_secret)) {
    $merchantId = $GLOBALS['config']->get('paypal_commerce', 'merchantId');
    if(empty($merchantId)) {
        $GLOBALS['main']->errorMessage("Onboarding failed. It's vital the 'Go back to CubeCart Limited' button is clicked to complete the setup process.");
        httpredir('?_g=plugins&type=plugins&module=paypal_commerce&disconnect=1');
        exit;
    }
    $connected = true;
