...
class Gateway {
	...
	public function call() {
		...
		if(!isset($headers['PAYPAL-TRANSMISSION-ID'])) {
			trigger_error('Failed to get PayPal Commerce webhook headers. Please check getallheaders() is available with your PHP handler. You can choose to skip verification via the "Show Advanced Settings" section of the extension configuration but that is approach is not recommended.', EXCEPTION);
		}
		$body = file_get_contents('php://input'); // JSON String
		if($this->_module['skip_webhook_verification']!=='1') {
			$this->_paypal = new paypalCheckout($this->_module);
			$verify = $this->_paypal->verifySignature($headers, $body);
		}

		if($this->_module['skip_webhook_verification']=='1' || (isset($verify['verification_status']) && $verify['verification_status']=='SUCCESS')) {
